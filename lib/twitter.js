...
Twitter.prototype._getUsingCursor = function(url, params, callback) {
  ...
  function fetch(err, data, rate_limit) {
    ...
    if (should_throttle) {
      ...
      seconds_until_window_ends = seconds_until_window_ends + 5;

      console.log('Delaying call for ' + seconds_until_window_ends + ' seconds');

      setTimeout(next_call, seconds_until_window_ends * 1000);
    } else {
      next_call();
    }
  }
