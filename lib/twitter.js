...
Twitter.TOO_MANY_REQUESTS_STATUS_CODE = 429;

module.exports = Twitter;

function calculateTimeUntilWindowEnds(rateLimitReset) {
  var now = Math.ceil(new Date().getTime() / 1000);
  var secondsUntilWindowEnds = Math.max(0, rateLimitReset - now);

  // 5 seconds after, just in case
  return (secondsUntilWindowEnds + 5) * 1000;
}
...
Twitter.prototype.get = function(url, params, callback) {
  ...
  function getCallback(error, data, response) {
    var rateLimit = (response && response.headers) ? {
      'reset': response.headers['x-rate-limit-reset'],
      'remaining': Number(response.headers['x-rate-limit-remaining']),
      'total': Number(response.headers['x-rate-limit-limit'])
    } : {};

    var json, isTooManyRequests = false;
    ...
    if (error && error.statusCode) {
      ...
      isTooManyRequests = (err.statusCode == Twitter.TOO_MANY_REQUESTS_STATUS_CODE);
    }
    else if (error) {
    ...
    if (!isTooManyRequests) {
      callback(err, json, rateLimit);
    } else if (rateLimit.reset > 0) { // if we know when the window ends
      // repeat call
      var timeUntilWindowEnds = calculateTimeUntilWindowEnds(rateLimit.reset);
      setTimeout(function repeatGet() {
        self.get(url, params, callback);
      }, timeUntilWindowEnds);

      console.log('Repeating call in ', Math.floor(timeUntilWindowEnds / 1000));
    } else {
      callback(error);
    }
  });
...
Twitter.prototype.showFriendship = function(source_id, target_id, callback) {
  var params = {
    source_id: source_id,
    target_id: target_id
  };
...
Twitter.prototype._getUsingCursor = function(url, params, callback) {
  ...
  function fetch(err, data, rateLimit) {
    if (err) {
      return callback(err);
    }
    ...
    if (data.next_cursor_str === '0') {
      nextCall = function getUsingCursorResult() {
        callback(null, result);
      };
    } else {
      nextCall = function nextGetUsingCursor() {
    ...
    if (shouldThrottle) {
      var timeToDelay = calculateTimeUntilWindowEnds(rateLimit.reset);
      setTimeout(nextCall, timeToDelay);

      console.log('Delaying call for ' + Math.floor(timeToDelay / 1000) + ' seconds');
    } else {
      nextCall();
    }
  }
